...
class GtfsFileController extends Controller {
    ...
    public function generateGtfsFiles($agency_id){
        ...
        // --------------- Calendar Dates
        $calendar_dates = CalendarDate::with('service')->where('agency_id', $agency_id)->get(['id', 'service_id', 'date', 'exception_type', 'created_at', 'updated_at']);
        $calendar_dates_data = 'service_id,date,exception_type' . PHP_EOL;
        foreach($calendar_dates as $calendar)
            $calendar_dates_data .= $this->clearString($calendar->service_id . ',' . date('Ymd', strtotime($calendar->date)) . ',' . $calendar->enum_key_exception_type) . PHP_EOL;
